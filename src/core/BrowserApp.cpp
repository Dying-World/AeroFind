...
LRESULT BrowserApp::HandleMessage(HWND window, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: ui_.Create(window, instance_); ui_.SetAddressEnterHandler([this] { NavigateFromAddress(); }); CreateMenu();
        ...
        RegisterHotKey(window, 9, MOD_CONTROL | MOD_SHIFT | MOD_NOREPEAT, 'B'); RegisterHotKey(window, 10, MOD_CONTROL | MOD_NOREPEAT, 'U'); RegisterHotKey(window, 11, MOD_CONTROL | MOD_SHIFT | MOD_NOREPEAT, 'I'); RegisterHotKey(window, 12, MOD_CONTROL | MOD_SHIFT | MOD_NOREPEAT, VK_SPACE); LoadSession(); StartEnvironment(); return 0;
    ...
    case WM_HOTKEY: if (wParam == 1) NewTab(); else if (wParam == 2) CloseTab(tabs_.ActiveIndex()); else if (wParam == 3) SwitchTab((tabs_.ActiveIndex() + 1) % tabs_.Count()); else if (wParam == 4) { SetFocus(ui_.AddressControl()); SendMessageW(ui_.AddressControl(), EM_SETSEL, 0, -1); } else if (wParam == 5 && webView_.Ready()) webView_.View()->Reload(); else if (wParam == 6) OpenRecords(L"history", L"Browsing history"); else if (wParam == 7) OpenRecords(L"downloads", L"Downloads"); else if (wParam == 8) SaveBookmark(); else if (wParam == 9) OpenRecords(L"bookmarks", L"Bookmarks"); else if (wParam == 10) ViewSource(); else if (wParam == 11) OpenDevTools(); else if (wParam == 12) ToggleFocusMode(); return 0;
    ...
    case WM_DESTROY: for (int id = 1; id <= 12; ++id) UnregisterHotKey(window, id); SaveSession(); webView_.Reset(); PostQuitMessage(0); return 0;
